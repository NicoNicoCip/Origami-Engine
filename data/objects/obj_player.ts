...
export class obj_player extends GameObject {
    ...
    step(): void {
        // Horizontal movement
        let hInput = 0;
        if (keyboard_check((globalThis as any).vk_a)) hInput -= 1;
        if (keyboard_check((globalThis as any).vk_d)) hInput += 1;
        ...
        // Jumping
        if (this.onGround && keyboard_check_pressed((globalThis as any).vk_space)) {
